...
def _verify_webhook_signature(
    ...
    if not secret:
        raise HTTPException(
            status_code=500,
            detail="Webhook route has no secret configured",
        )
...
def create_app(
    adapter: VoiceOSGatewayAdapter,
    gateway_config,
) -> FastAPI:
    app = FastAPI(
        title="VoiceOS Gateway",
        version="1.0.0",
    )
    ...
    routes: Dict[str, Any] = (
        getattr(gateway_config, "webhooks", {}) or {}
    )
    ...
    async def webhook(
        ...
        x_voiceos_signature: Optional[str] = Header(
            default=None,
            alias="X-VoiceOS-Signature",
        ),
    ) -> Dict[str, Any]:
        ...
        if route is None:
            raise HTTPException(
                status_code=404,
                detail=f"Unknown webhook route: {route_name}",
            )
